...
public:
    ...
    __Tp *getDeviceMemPointer() {
        return (__Tp *) dev_mem;
    }

    template<typename __Tp>
    __Tp *getHostMemPointer() {
...
public:
    ...
    int createOnZero(std::string name, size_t size_in_bytes) {
        if (gpu_mmap.find(name) != gpu_mmap.end()) {
            std::cout << "createOnDevice::Failure to export data with the name \"" + name +
                         "\". The name already exists in the GPU memory map." << std::endl;
            L_(lerror) << "createOnDevice::Failure to export data with the name \"" + name +
                          "\". The name already exists in the GPU memory map.";
            return EXIT_FAILURE;
        }
    ...
    int copyToZero(std::string name, void *src_data, size_t size_in_bytes) {
        ...
        if (!src_data) {
            ...
            L_(lerror) << "copyToZero::Failure to copy data from host source address; it is a nullptr.";
            return EXIT_FAILURE;
        }
    ...
    int copyFromZero(std::string name, void *dst_data, size_t size_in_bytes) {
        ...
        if (it == gpu_mmap.end()) {
            std::cout << "copyFromZero::Failure to find host entry with the name \"" + name +
                         "\". It does not exist in the host memory map." << std::endl;
            L_(lerror) << "copyFromZero::Failure to find host entry with the name \"" + name +
                          "\". It does not exist in the host memory map.";
            return EXIT_FAILURE;
        }
        CUDADevice_ArrayMappedMemory &gpu_mem = it->second;
        if (!dst_data) {
            std::cout << "copyFromZero::Failure to copy. The host destination address is a nullptr." << std::endl;
            L_(lerror) << "copyFromZero::Failure to copy. The host destination address is a nullptr.";
            return EXIT_FAILURE;
        }
        if (!gpu_mem.host_mem) {
            std::cout << "copyFromZero::Failure to copy. The device source address is a nullptr." << std::endl;
            L_(lerror) << "copyFromZero::Failure to copy. The device source address is a nullptr.";
            return EXIT_FAILURE;
        }
        if (size_in_bytes > gpu_mem.size) {
            std::cout
                    << "copyFromZero::Failure to copy. The destination address memory size is larger than the device source size."
                    << std::endl;
            L_(lerror)
                << "copyFromZero::Failure to copy. The destination address memory size is larger than the device source size.";
            return EXIT_FAILURE;
            ...
            std::cout
                    << "copyFromZero::Warning. The destination address memory size is smaller than the device source size."
                    << std::endl;
            L_(lerror) << "copyFromZero::Warning. The destination address memory size is smaller than the device source size.";
        }
    ...
    template<typename __Tp>
    __Tp *getHostMemPointer(std::string name) {
        mmap_iterator it = gpu_mmap.find(name);
        if (it == gpu_mmap.end()) {
            std::cout << "getHostMemPointer::Failure to find GPU entry with the name \"" + name +
                         "\". It does not exist in the GPU memory map." << std::endl;
        }
        return (__Tp *) it->second.host_mem;
    }

    template<typename __Tp>
    __Tp *getDeviceMemPointer(std::string name) {
        mmap_iterator it = gpu_mmap.find(name);
        if (it == gpu_mmap.end()) {
            std::cout << "getDeviceMemPointer::Failure to find GPU entry with the name \"" + name +
                         "\". It does not exist in the GPU memory map." << std::endl;
        }
    ...
    int copyFromDevice(std::string name, void *dst_data, size_t size_in_bytes) {
        mmap_iterator it = gpu_mmap.find(name);
        if (it == gpu_mmap.end()) {
            std::cout << "copyFromDevice::Failure to find GPU entry with the name \"" + name +
                         "\". It does not exist in the GPU memory map." << std::endl;
            return EXIT_FAILURE;
        }
        CUDADevice_ArrayMappedMemory &gpu_mem = it->second;
        ...
        if (size_in_bytes > gpu_mem.size) {
            std::cout
                    << "copyFromDevice::Failure to copy. The destination address memory size is larger than the device source size."
                    << std::endl;
            return EXIT_FAILURE;
        } else if (size_in_bytes < gpu_mem.size) {
            std::cout
                    << "copyFromDevice::Warning. The destination address memory size is smaller than the device source size."
                    << std::endl;
        }
    ...
    int freeGPUMemory(std::string name) {
        mmap_iterator it = gpu_mmap.find(name);
        if (it == gpu_mmap.end()) {
            std::cout << "freeGPUMemory::Failure to free. GPU memory with the name \"" + name +
                         "\" does not exist in the GPU memory map." << std::endl;
            return EXIT_FAILURE;
        }
        CUDADevice_ArrayMappedMemory &gpu_mem = it->second;
        if (!gpu_mem.dev_mem) {
            std::cout << "freeGPUMemory::Failure to free. GPU memory with the name \"" + name +
                         "\" has device memory address = nullptr." << std::endl;
        }
    ...
    int freeZeroMemory(std::string name) {
        mmap_iterator it = gpu_mmap.find(name);
        if (it == gpu_mmap.end()) {
            std::cout << "freeHostMemory::Failure to free. Host memory with the name \"" + name +
                         "\" does not exist in the host memory map." << std::endl;
            return EXIT_FAILURE;
        }
        CUDADevice_ArrayMappedMemory &gpu_mem = it->second;
        if (!gpu_mem.host_mem) {
            std::cout << "freeHostMemory::Failure to free. GPU memory with the name \"" + name +
                         "\" has device memory address = nullptr." << std::endl;
        }
    ...
    int freeHostMemory(std::string name) {
        ...
        if (!gpu_mem.host_mem) {
            std::cout << "freeHostMemory::Failure to free. Host memory with the name \"" + name +
                         "\" has host memory address = nullptr." << std::endl;
        }
    ...
    int freeAllResources() {
        ...
        for (const auto it: gpu_mmap) {
            std::cout << "Freeing GPU-Host memory for array " << it.first << " : " << it.second.size << " bytes freed."
                      << std::endl;
...
void cufft_engine(cufftComplex *dev_signal, int N_fft, int N_batch, int DIR) {
    ...
    if (cufftPlan1d(&plan, N_fft, CUFFT_C2C, N_batch) != CUFFT_SUCCESS) {
        ...
        L_(lerror) << "CUFFT error: Plan creation failed";
        return;
    }
    ...
    L_(linfo) << "Transforming signal cufftExecC2C";
    if (cufftExecC2C(plan, (cufftComplex *) dev_signal, (cufftComplex *) dev_signal, DIR) != CUFFT_SUCCESS) {
        ...
        L_(lerror) << "CUFFT error: ExecC2C Forward failed";
    }
}
...
int initialize_CUDAResources(const SAR_Aperture<__nTp1> &sar_data,
    ...
    std::cout << "Device " << cuda_res.deviceId << ": \"" << cuda_res.props.name
              << " has NVIDIA \"" << _ConvertSMVer2ArchName(cuda_res.props.major, cuda_res.props.minor)
              << "\" architecture" << " having compute capability " <<
              cuda_res.props.major << "." << cuda_res.props.minor << "." << std::endl;
...
int finalize_CUDAResources(const SAR_Aperture<__nTp1> &sar_data,
    ...
    if (this_error != cudaSuccess) {
        ...
        L_(lerror) << "ERROR: cudaGetLastError did NOT return success! DO NOT TRUST RESULTS!";
        L_(lerror) << "CUDAERROR = " << cudaGetErrorString(this_error);
    }
...
void cuda_focus_SAR_image(const SAR_Aperture<__nTp> &sar_data,
    ...
    switch (sar_image_params.algorithm) {
        case SAR_ImageFormationParameters<__nTpParams>::ALGORITHM::BACKPROJECTION:
            ...
            L_(linfo) << "Selected backprojection algorithm for focusing.";
        ...
        case SAR_ImageFormationParameters<__nTpParams>::ALGORITHM::MATCHED_FILTER:
            ...
            L_(linfo) << "Selected matched filtering algorithm for focusing.";
        ...
        default:
            ...
            L_(lerror) << "focus_SAR_image()::Algorithm requested is not recognized or available.";
            return;
    }
    ...
    L_(linfo) << "Maximum Scene Size:  " << std::fixed << std::setprecision(2) << sar_image_params.max_Wy_m
              << " m range, "
              << sar_image_params.max_Wx_m << " m cross-range";
    L_(linfo) << "Maximum Resolution:  " << std::fixed << std::setprecision(2) << sar_image_params.slant_rangeResolution
              << "m range, "
              << sar_image_params.azimuthResolution << " m cross-range";
    GPUMemoryManager cuda_res;
    ...
    L_(linfo) << "CUDA FFT kernels took " << (float) (c1 - c0) * 1000 / CLOCKS_PER_SEC << " ms.";
    ...
    L_(linfo) << "CUDA Backprojection kernel launch took " << (float) (c1 - c0) * 1000 / CLOCKS_PER_SEC << " ms.";
    if (cudaDeviceSynchronize() != cudaSuccess) {
        printf("\nERROR: threads did NOT synchronize! DO NOT TRUST RESULTS!\n\n");
        L_(lerror) << NowTime << "::" << "threads did NOT synchronize! DO NOT TRUST RESULTS!";
    }
    c2 = clock();
    printf("INFO: CUDA Backprojection execution took %f ms.\n", (float) (c2 - c1) * 1000 / CLOCKS_PER_SEC);
    L_(linfo) << "CUDA Backprojection execution took " << (float) (c2 - c1) * 1000 / CLOCKS_PER_SEC << " ms.";
    L_(linfo) << "CUDA Backprojection total time took " << (float) (c2 - c0) * 1000 / CLOCKS_PER_SEC << " ms.";
#if ZEROCOPY
    ...
    if (finalize_CUDAResources(sar_data, sar_image_params, cuda_res) == EXIT_FAILURE) {
        std::cout << "cuda_focus_SAR_image::Problem found de-allocating and free resources on the GPU. Exiting..."
                  << std::endl;
        L_(lerror) << "cuda_focus_SAR_image::Problem found de-allocating and free resources on the GPU. Exiting...";
        return;
    }
    std::cout << cuda_res << std::endl;
}
