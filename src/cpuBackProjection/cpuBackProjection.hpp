...
Complex<__nTp> interp1(const __nTp *xSampleLocations, const int nSamples, const CArray<__nTp> &sampleValues,
                       const float xInterpLocation, const __nTp xIndex) {
    ...
    if (rightIdx == nSamples || rightIdx == 0) {
        ...
        L_(lerror) << "Error::Invalid interpolation range.";
        return iVal;
    }
    ...
    //}
    __nTp alpha = (xInterpLocation - xSampleLocations[rightIdx - 1]) /
                  (xSampleLocations[rightIdx] - xSampleLocations[rightIdx - 1]);
...
void computeDifferentialRangeAndPhaseCorrections(int pulseIndex,
    ...
    for (int xIdx = 0; xIdx < SARImgParams.N_x_pix; xIdx++) {
        ...
        for (int yIdx = 0; yIdx < SARImgParams.N_y_pix; yIdx++) {
            float dR_val = std::sqrt(
                    (SARData.Ant_x.data[pulseIndex] - target_x) * (SARData.Ant_x.data[pulseIndex] - target_x) +
                    (SARData.Ant_y.data[pulseIndex] - target_y) * (SARData.Ant_y.data[pulseIndex] - target_y) +
                    (SARData.Ant_z.data[pulseIndex] - target_z) * (SARData.Ant_z.data[pulseIndex] - target_z)) -
                           SARData.slant_range.data[pulseIndex];
            ...
            if (dR_val > min_Rvec && dR_val < max_Rvec) {
                ...
                Complex<__nTp> phCorr_val = Complex<__nTp>::polar(1.0f, (__nTp) (
                        (4.0 * PI * SARData.startF.data[pulseIndex] * dR_val) / CLIGHT));
...
void computeDifferentialRangeAndPhaseCorrectionsMF(int pulseIndex,
    ...
    for (int xIdx = 0; xIdx < sar_image_params.N_x_pix; xIdx++) {
        ...
        for (int yIdx = 0; yIdx < sar_image_params.N_y_pix; yIdx++) {
            float dR_val = std::sqrt(
                    (sar_data.Ant_x.data[pulseIndex] - target_x) * (sar_data.Ant_x.data[pulseIndex] - target_x) +
                    (sar_data.Ant_y.data[pulseIndex] - target_y) * (sar_data.Ant_y.data[pulseIndex] - target_y) +
                    (sar_data.Ant_z.data[pulseIndex] - target_z) * (sar_data.Ant_z.data[pulseIndex] - target_z)) -
                           sar_data.slant_range.data[pulseIndex];
            //  std::cout << "y= " << target.y << " dR(" << xIdx << ", " << yIdx << ") = " << dR_val << std::endl;
            //int outputIdx = xIdx * SARImgParams.N_y_pix + yIdx;
            //std::cout << "output[" << outputIdx << "] += " << (iRC_val * phCorr_val) << std::endl;
            ...
            for (int freqIdx = 0; freqIdx < sar_data.numRangeSamples; freqIdx++) {
                ...
                Complex<__nTp> phCorr_val = Complex<__nTp>::polar(1.0f, (__nTp) (
                        (4.0 * PI * sar_data.freq.data[pulse_startF_FreqIdx + freqIdx] * dR_val) / CLIGHT));
...
void run_mf(const SAR_Aperture<__nTp> &SARData,
    ...
    L_(linfo) << "Running matched filter SAR focusing algorithm.";
    __nTp timeleft = 0.0f;
    ...
    for (int pulseIndex = 0; pulseIndex < SARData.numAzimuthSamples; pulseIndex++) {
        if (pulseIndex > 1) {// && (pulseIndex % 100) == 0) {
            L_(linfo) << "Pulse " << pulseIndex << " of " << SARData.numAzimuthSamples
                      << ", " << std::setprecision(2) << timeleft << " minutes remaining";
...
void focus_SAR_image(const SAR_Aperture<__nTp> &SARData,
    ...
    L_(linfo) << "Maximum Scene Size:  " << std::fixed << std::setprecision(2) << SARImgParams.max_Wy_m << " m range, "
              << SARImgParams.max_Wx_m << " m cross-range";
    std::cout << "Resolution:  " << std::fixed << std::setprecision(2) << SARImgParams.slant_rangeResolution
              << "m range, " << SARImgParams.azimuthResolution << " m cross-range" << std::endl;
    L_(linfo) << "Resolution:  " << std::fixed << std::setprecision(2) << SARImgParams.slant_rangeResolution
              << "m range, "  << SARImgParams.azimuthResolution << " m cross-range";
